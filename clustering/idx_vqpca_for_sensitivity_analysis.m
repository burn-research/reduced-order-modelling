...
while ((convergence == 0) && (iter < iter_max))
    ...
    for j = 1:1:k
        ...
        [n_obs_local_X, n_vars_local_X] = size(centered_scaled_local_X);
        
        % PCA with eigendecomposition of the covariance matrix:
        if strcmp(pca_algorithm, 'cov-eig')
            S = 1/(n_obs_local_X - 1) .* centered_scaled_local_X' * centered_scaled_local_X;
            [modes, eigenvalues] = eig(S);
        ...
        if strcmp(pca_algorithm, 'cov-svd')
            S = 1/(n_obs_local_X - 1) .* centered_scaled_local_X' * centered_scaled_local_X;
            [U, eigenvalues, modes] = svd(S);
            eigvec{j} = modes(:,1:n_eigs);
        end

        % PCA with SVD of the data matrix:
        if strcmp(pca_algorithm, 'X-svd')
            [U, eigenvalues, modes] = svd(centered_scaled_local_X, 'econ');
            eigvec{j} = modes(:,1:n_eigs);
        end

        % PCA builtin Matlab function that does SVD by default:
        if strcmp(pca_algorithm, 'builtin')
            [modes, ~, eigenvalues] = pca(centered_scaled_local_X, 'Centered', false);
            eigvec{j} = modes(:,1:n_eigs);
        end
