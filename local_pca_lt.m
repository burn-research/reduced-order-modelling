...
function [idx, eigvec, eps_rec_new_clust, nz_X_k] = local_pca_lt(X, names, k, n_eigs, C)
...
pre_cent_crit = 0; %Refer to center.m for values
pre_scal_crit = 1; %Refer to scale.m for values
...
% Initialize the effective number of clusters as the original number
k_new = k;
...
%     end
    ...
    sq_err_clust = partition(rec_err_min, idx, k);
    ...
    if VQ == 1
        ...
        [nz_X_k, nz_idx_clust]  = partition(scal_X, idx, k);
    ...
    rec_err_min_rel_k = cell(k_new, 1);
    for j = 1 : k_new
        rec_err_min_rel_k{j} = rec_err_min_rel(nz_idx_clust{j}, 1);
    end
    
     % Evaluate the mean error in each cluster
    eps_rec_new_clust = zeros(k_new, 1);
    size_clust = zeros(k_new, 1);
    rec_err_clust = zeros(k_new, 1);
    ...
    mean_rec_err = mean(rec_err_clust);
    ...
    if VQ == 1
        ...
        sort_eigval = cell(k_new, 1);
        sort_eigvec = cell(k_new, 1);
        eigval = cell(k_new, 1);
        eigvec = cell(k_new, 1);
        n_eig = cell(k_new, 1);
        U_scores = cell(k_new, 1);
        W_scores = cell(k_new, 1);
        gamma = cell(k_new, 1);
        scaled_data = cell(k_new, 1);
        rec_data = cell(k_new, 1);

        for j = 1 : k_new
            [sort_eigval{j}, sort_eigvec{j}, eigval{j}, eigvec{j}, n_eig{j}, ...
                U_scores{j}, W_scores{j}, gamma{j}, scaled_data{j}, rec_data{j}] ...
                = pca_lt(nz_X_k{j}, cent_crit, scal_crit, 6);
        end
        iter = iter + 1;
    elseif VQ == 2
        convergence = 1;    
